...
class SkillRanker:
    ...
    @staticmethod
    def _resolve_embedding_api() -> tuple[Optional[str], str]:
        """Resolve OpenAI-compatible API key and base URL for embedding."""
        import os
        or_key = os.environ.get("OPENROUTER_API_KEY")
        if or_key:
            return or_key, "https://openrouter.ai/api/v1"
        oa_key = os.environ.get("OPENAI_API_KEY")
        if oa_key:
            return oa_key, "https://api.openai.com/v1"
        try:
            from openspace.host_detection import get_openai_api_key
            host_key = get_openai_api_key()
            if host_key:
                return host_key, "https://api.openai.com/v1"
        except Exception:
            pass
    ...
    def _get_openai_api_key() -> Optional[str]:
        ...
        api_key, _ = SkillRanker._resolve_embedding_api()
        return api_key
    ...
    def _generate_embedding(
        ...
        resolved_key, base_url = SkillRanker._resolve_embedding_api()
        if not api_key:
