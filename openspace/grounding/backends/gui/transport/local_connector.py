    ...
    def _get_screenshot_helper(self):
        if self._screenshot_helper is None:
            try:
                ...
                raise ImportError(
                    "ScreenshotHelper requires openspace local_server module. "
                    "Install the full OpenSpace package or use the HTTP GUIConnector."
                )
    ...
    def _get_accessibility_helper(self):
        if self._accessibility_helper is None:
            try:
                ...
                raise ImportError(
                    "AccessibilityHelper requires openspace local_server module. "
                    "Install the full OpenSpace package or use the HTTP GUIConnector."
                )
